...
func TestTransparentRGB565FastPathMatchesReference(t *testing.T) {
	...
	// Alternate transparent pixels, short opaque runs and long opaque runs so
	// the span scanner exercises both skipping and copy batching.
	...
	if !blitTransparentRGB565FastPath(
		destination,
		source,
...
func TestScaledBlitTransparentRGB565UsesBulkPath(t *testing.T) {
	...
	check(t, err)
	key := RGB(0, 0, 0)
	destinationID, err := graphics.CreateSurface(1, SurfaceDescriptor{
		Width: 32, Height: 24, Format: PixelRGB565,
	})
	check(t, err)
	sourceID, err := graphics.CreateSurface(1, SurfaceDescriptor{
		Width: 16, Height: 16, Format: PixelRGB565, Transparent: &key,
	})
	check(t, err)
	...
	if graphics.bulkFastPathCalls != before+1 {
		t.Fatalf("RGB565 transparent blit fast-path count = %d, want %d",
			graphics.bulkFastPathCalls, before+1)
	}
}
...
func TestScaledBlitStreamingSlowPathMatchesReference(t *testing.T) {
	...
	check(t, err)
	destinationID, err := graphics.CreateSurface(1, SurfaceDescriptor{
		Width: 20, Height: 18, Format: PixelRGBA8888,
	})
	check(t, err)
	sourceID, err := graphics.CreateSurface(1, SurfaceDescriptor{
		Width: 12, Height: 10, Format: PixelRGBA8888,
	})
	check(t, err)
	...
	for index := range destination.pixels {
		destination.pixels[index] = byte(index*19 + 3)
	}
	for index := range source.pixels {
		source.pixels[index] = byte(index*31 + 11)
	}
	makeOpaque(source)
	destination.state.GlobalAlpha = 0x80 // forces the non-fast streaming path
